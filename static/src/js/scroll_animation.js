...
publicWidget.registry.ScrollAnimationSection = publicWidget.Widget.extend({
  ...
  start: function () {
    ...
    this._resizeHandler = this._debounce(() => {
      ...
      if (wasDesktop !== this.isDesktop) {
        console.log(
          `Screen size changed: ${
            this.isDesktop ? "Desktop" : "Mobile/Tablet"
          } mode`
        );
        this._handleResponsiveInit();
      }
   ...
   * Using Motion.js scroll() API for smooth, performant scroll-jacking
   */
  _initScrollAnimation: function () {
    ...
    if (typeof Motion !== "undefined" && Motion.scroll) {
      console.log("Using Motion.js scroll() API for scroll-jacking");
      this._initMotionScrollAnimation(
        section,
        textStates,
        visualStates,
        progressDots,
      ...
      console.warn("Motion.js not available, using vanilla JS fallback");
      this._initVanillaScrollAnimation(
        section,
        textStates,
        visualStates,
        progressDots,
  ...
  /**
   * Motion.js scroll() implementation - Clean & Powerful
   * Enhanced with smooth progress tracking and parallax effects
   */
  _initMotionScrollAnimation: function (
    section,
    textStates,
    visualStates,
    progressDots,
    ...
    // Use Motion.js scroll() to track progress through the section
    const scrollAnimation = Motion.scroll(
      (progress) => {
        // Safely get scroll progress (handle undefined)
        const scrollProgress =
          typeof progress === "number"
            ? progress
            : progress && typeof progress.y === "number"
            ? progress.y
            : 0;

        // Calculate state index (0 to totalStates-1)
        // More granular: each state gets equal scroll range
        let stateIndex;
        if (scrollProgress >= 0.98) {
          // Ensure last state is reachable
          stateIndex = totalStates - 1;
        } else {
          // Distribute states evenly across scroll progress
          stateIndex = Math.floor(scrollProgress * totalStates);
          stateIndex = Math.max(0, Math.min(stateIndex, totalStates - 1));
        }

        // Only update if state changed
        if (stateIndex !== currentState) {
          console.log(
            `[Motion.js] State: ${currentState} → ${stateIndex} (progress: ${
        ...
        // Add parallax effect to visual gradients
        this._applyParallaxEffects(section, scrollProgress);
      },
      {
        target: section,
  ...
  /**
   * Apply parallax effects to visual elements during scroll
   */
  _applyParallaxEffects: function (section, progress) {
    if (typeof progress !== "number" || progress < 0 || progress > 1) return;

    // Apply subtle parallax to gradients
    const gradients = section.querySelectorAll(".visual-gradient");
    gradients.forEach((gradient, index) => {
      const offset = (index % 2 === 0 ? 1 : -1) * progress * 30;
      gradient.style.transform = `translateY(${offset}px) scale(${
        1 + progress * 0.1
      })`;
    });

    // Apply floating animation to icons
    const icons = section.querySelectorAll(".visual-elements iconify-icon");
    icons.forEach((icon, index) => {
      const floatOffset = Math.sin(progress * Math.PI * 2 + index) * 10;
  ...
  /**
   * Vanilla JS fallback (if Motion.js not available)
   */
  _initVanillaScrollAnimation: function (
    section,
    textStates,
    visualStates,
    progressDots,
    ...
    const updateState = () => {
      ...
      if (stateIndex !== currentState && !isAnimating) {
        console.log(
          `[Vanilla] State: ${currentState} → ${stateIndex} (progress: ${progress.toFixed(
            2
          )})`
        );
        isAnimating = true;
  ...
  _initProgressIndicator: function () {
    ...
    progressDots.forEach((dot, index) => {
      dot.addEventListener("click", () => {
        ...
        const scrollableHeight = sectionHeight - viewportHeight;
        const targetScroll =
          sectionTop + (scrollableHeight * index) / progressDots.length;
